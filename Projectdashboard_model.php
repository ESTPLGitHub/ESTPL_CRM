...
LEFT JOIN pre_construction_activities_encroachment_eviction AS ev ON pcs.project_id = ev.project_id WHERE pcs.project_id='".$project_id."'");
   ...
   public function get_individual_data($project_id) {
    ...
    {
        $query = $this->db->query("SELECT IFNULL(SUM(target_amount), 0) as Planned_Value, IFNULL(SUM(earned_amount), 0) as Earned_Value, IFNULL(SUM(allotted_amount), 0) as Paid_Value FROM `project_financial_planning_detail` WHERE project_id=".$project_id." ");
        $result = $query->result_array();
        return $result;
    }
